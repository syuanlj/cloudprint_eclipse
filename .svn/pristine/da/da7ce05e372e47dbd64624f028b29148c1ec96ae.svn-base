...
public class MQTTClient implements Runnable {
	...
	public void run() {
		...
		// String host = "tcp://localhost:61613";
		String host = "tcp://139.129.27.116:1883";
		// String host = "tcp://123.57.57.102:61613";
		String userName = "admin";
		String passWord = "public";
		// String host = "tcp://59.56.74.247:22";
		// String userName = "server1";
		// String passWord = "12345677";
		try {
			final MqttClient client = new MqttClient(host, "printer" + i);
							...
							if (bodyLength == payload.length - PrintProtocol.BODY_OFFSET) {
								...
								if (checksum == checksum(payload, PrintProtocol.BODY_OFFSET)) {
									...
									if (PrintProtocol.COMMAND_ID_PUSH_ORDER == commandId) {
										...
										client.getTopic(PrintTopic.prefix_up + "printer" + i).publish(message);
										...
										client.getTopic(PrintTopic.prefix_up + "printer" + i).publish(message);
										System.out.println(new Date(System.currentTimeMillis()) + "----------command id----------" + PrintProtocol.COMMAND_ID_PRINT_ORDER_FEEDBACK + "----------product sn----------" + productSn + "----content----" + content);
			...
			client.subscribe(PrintTopic.prefix_down + "printer" + i);
			System.out.println("printer" + i + " connected.");
