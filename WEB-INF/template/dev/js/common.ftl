...
(function($) {
	...
	$.message = function() {
		...
		if ($message == null) {
			$message = $('<div class="cloudprintMessage"><div class="messageContent message' + message.type + 'Icon"><\/div><\/div>');
	...
	$.dialog = function(options) {
		...
		var $dialog = $('<div class="cloudprintDialog"><\/div>');
		var $dialogTitle;
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: cloudprint.base + "/admin/file/browser.ehtml",
				uploadUrl: cloudprint.base + "/admin/file/upload.ehtml",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					var $browser = $('<div class="cloudprintBrowser"><\/div>');
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = cloudprint.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = cloudprint.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
